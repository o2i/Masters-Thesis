...
def gen_parameters(df_demand, df_parking, interval):
...
def optimize(df_demand, df_parking, sens, interval):

    # Import i and j set function
    demand_lc, chg_lc = gen_sets(df_demand, df_parking)

    # Import parameters function
    di, m, p, t, ci_j, cr_j, ce_j, pe, alpha, lj, distance_matrix = gen_parameters(df_demand, df_parking, interval)
    ...
    #N = 10
    df_demand['insentive'] = 1437500/365*interval                 # ce_j represents the price of a charger in station j
...
#     opt_loc_df2.to_csv(path_or_buf='optimal_locations.csv')
    ...
    print(demand)'''
    dtf = pd.DataFrame([{'obj':prob.objective.value(),'Ij':interval}])

    return dtf #opt_location, df_status,
#df_demand, df_parking, v0val, uval, peval, ljval, alphaval, mval, pval, tval, cijval, crjval, cejval

sens = pd.DataFrame()
for k in range(0,8):
    interval = 1/7*k
    sens = pd.concat([sens,optimize(GIS_df, car_park_df, sens, interval)])
sens.to_csv("sensitivity/Ij.csv")   
#plt.plot(sens['lj'], sens['obj'])
plt.show()
print(sens)
